...
static int cass_best_cpu(struct task_struct *p, int prev_cpu, bool sync)
{
	 ...
	 */
	rcu_read_lock();
	struct for_each_cpu_and(cpu, &p->nr_cpus_allowed, cpu_active_mask) {
...
struct int cass_select_task_rq_fair(struct task_struct *p, int prev_cpu,
				    ...
				    int sibling_count_hint)
{
	 ...
	 * on inactive CPUs.
	 */
	if (unlikely(!cpumask_intersects(&p->nr_cpus_allowed, cpu_active_mask)))
		return cpumask_first(&p->nr_cpus_allowed);
